...
In the amorphous sample (0ns) formation of small islands of size \mremove{$4\pm4$}\madd{four plus or minus four} molecules begins. At \replace{30\%}{thirty percent} crystallinity these islands become connected resulting in the formation of 
elongated \replace{1D}{one dimensional} paths, which extend to \replace{2D}{two dimensional} clusters at \replace{60\%}{sixty percent} crystallinity. At \replace{80\%}{eighty percent} crystallinity these clusters grow to \mremove{$9\pm16$}{nine plus or minus sixteen} molecules, but still short of the formally infinitely 
...
Finally, in figure \ref{fig:crystalCouplingGraph}, the upper row shows a representative wavefunction for each of the systems studied. In the first image (\replace{0\%}{zero percent} crystallinity) we see the wavefunction localised primarily on a single molecule with a small tail its neighbours. As the crystallinity increases so too does the delocalisation of the wavefunction, until eventually, in the single crystal sample (\replace{100\%}{one hundred percent} crystallinity) the wavefunction is delocalised over \replace{17}{seventeen} molecules.
